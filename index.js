...
//🚀🚀🚀 ⬇️ 📝 Explanation ⬇️ 📝 🚀🚀🚀: 
// Its because a nested fuction can reach out of its designanted scope and use the variable internal. 
...
function summation(pram) {
  let array = []
  for(let i = pram; i > 0 ; i--){
    array.push(i)
}
...
const zooAnimals = [
 ...
 function animalNames(zooAnimals){
  let arr = []
 const zooo = zooAnimals.forEach(function(item){
 ...
 function lowerCaseNames(zooAnimals){
  const maaap1 = zooAnimals.map(function(item){
...
return maaap1
 ...
 function lowPopulationAnimals(zooAnimals){
  const result = zooAnimals.filter(anime => anime.population < 5);
  return result
}
 ...
 function USApop(zooAnimals){
  ...
  const sum = zooAnimals.reduce(function (a, b) {
    return a + b.population; // returns object with property x
  },0)
